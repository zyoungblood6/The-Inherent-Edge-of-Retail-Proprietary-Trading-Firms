...
PATH_PNL_OUTPUT_FILE = "raw_10am_100tick_8mnq_10000_path_pnl.csv"
...
# ============================================================
# EXPORT ALL 10,000 FULL-SAMPLE RAW PATH P&Ls
# ============================================================
#
# This does NOT change the simulation or any previously reported
# result. It only saves the already-generated full-sample path
# outcomes so they can be plotted as a histogram/exhibit.
# ============================================================

full_long_pnl = clean_days["long_pnl"].to_numpy(dtype=float)
full_short_pnl = clean_days["short_pnl"].to_numpy(dtype=float)

full_chosen_pnl = np.where(
    paths == 1,
    full_long_pnl[None, :],
    full_short_pnl[None, :]
)

full_path_total_pnl = full_chosen_pnl.sum(axis=1)

raw_path_pnl_df = pd.DataFrame({
    "simulation": np.arange(1, NUM_SIMULATIONS + 1),
    "raw_total_pnl": full_path_total_pnl
})

raw_path_pnl_df.to_csv(
    PATH_PNL_OUTPUT_FILE,
    index=False
)
